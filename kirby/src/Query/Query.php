...
class Query
{
	...
	private function resolve_runner(Runner $runner, array|object $data = []): mixed
	{
		return $runner->run($this->query, $data);
	}
	...
	public function resolve(array|object $data = []): mixed
	{
		...
		switch(option('query.runner', 'legacy')) {
		case 'interpreted':
			$runner = new Interpreted(static::$entries);
			break;
		case 'transpiled':
			$runner = new Transpiled(static::$entries);
			break;
		default:
			return $this->resolve_legacy($data);
		}

		return $this->resolve_runner($runner, $data);
	}
}
